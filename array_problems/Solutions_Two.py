...
"""
Here i'm going to traverse throughout the board until I detect a battleship, i'll increment a result counter 
and dfs traverse through the length of the battleship marking as I go.  I'll then return the number of detects.
This will be O(n) time complexity and O(max(len(board)) space complexity due to the recursive nature of thee problem.
"""
...
def intervalIntersection(firstList: List[List[int]], secondList: List[List[int]]) -> List[List[int]]:
    ...
    while pointer_a < len(firstList) and pointer_b < len(secondList):
        ...
        if secondList[pointer_b][0] <= firstList[pointer_a][0] <= secondList[pointer_b][1]:
            result.append([firstList[pointer_a][0], min(secondList[pointer_b][1], firstList[pointer_a][1])])
        elif firstList[pointer_a][0] <= secondList[pointer_b][0] <= firstList[pointer_a][1]:
            result.append([secondList[pointer_b][0], min(firstList[pointer_a][1], secondList[pointer_b][1])])
...
"""
This is a recursive problem where I recursively iterate on the remaining permutation space until I reach a final solution space.  I aggregate the solution space int oa result.
...
"""
Here I will traverse the nums space iteratively and build a results along the way.  These will be added to a result list
...
"""
In this problem I will traverse the grid until I reach land.  I'll then aggregate all adjacent land pieces in a dfs manner.
Then I'll return the total area of this aggregation.  I'll finally return the maximal value of these aggregations.
...
Each element in the result matrix will be ans[x][y] = min(ans[x-1][y-1],ans[x-1][y],ans[x-1][y+1]) + matrix[x][y]
...
This will be an O(n^2) operation as it may be a differnet answer for each one and it can potentially 
traverse every node.
"""
...
"""
Here I will recursively traverse the array starting at 0, i'll go in either direction and maintain a 'visited' set.  If I haven't visited an area i'll continue traversing.
